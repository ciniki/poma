...
// <rsp stat='ok' />
//
function ciniki_poma_favItemDelete(&$ciniki, $business_id, $args) {
    ...
    if( isset($rc['rows']) && count($rc['rows']) > 0 ) {
        foreach($rc['rows'] as $row) {
            ...
            $rc = ciniki_core_objectDelete($ciniki, $business_id, 'ciniki.poma.customeritem', $row['id'], $row['uuid'], 0x07);
            if( $rc['stat'] != 'ok' ) {
                return $rc;
            }
        }
