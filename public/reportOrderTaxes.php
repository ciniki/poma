...
// -------
//
function ciniki_poma_reportOrderTaxes(&$ciniki) {
    //  
    ...
    //
    // FIXME: Get year end from poma settings
    //
    $year_end_month = 12;
    $year_end_day = 31;
    ...
    //
    $strsql = "SELECT ciniki_poma_orders.order_date, "
        . "ciniki_poma_order_taxes.id, "
        . "ciniki_poma_order_taxes.taxrate_id, "
        . "ciniki_poma_order_taxes.amount "
    ...
    $rc = ciniki_core_dbHashQueryArrayTree($ciniki, $strsql, 'ciniki.poma', array(
        array('container'=>'taxes', 'fname'=>'id', 
            'fields'=>array('order_date', 'taxrate_id', 'amount'),
            'utctodate'=>array('order_date'=>$intl_timezone),
            ),
        ));
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    ...
    $quarters = array();
    if( count($taxes) > 0 ) {
        $start_date = clone $taxes[0]['order_date'];
        ...
        foreach($taxes as $tid => $tax) {
            if( $tax['order_date'] > $cur_date ) {
                break;
            }
            while( $tax['order_date'] > $quarters[$current_quarter]['end_date'] ) {
                $current_quarter++;
    ...
    return array('stat'=>'ok', 'taxrates'=>$r_taxrates, 'tax_quarters'=>$quarters);
}
?>
