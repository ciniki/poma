...
function ciniki_poma_noteUpdate(&$ciniki) {
    ...
    $rc = ciniki_poma_checkAccess($ciniki, $args['tnid'], 'ciniki.poma.noteUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.poma.note', $args['note_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.poma.note', 'object_id'=>$args['note_id']));
