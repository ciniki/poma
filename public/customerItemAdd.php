...
// Description
// -----------
// This method will add a new customer item for the business.
//
// Arguments
// ---------
// api_key:
// auth_token:
// business_id:        The ID of the business to add the Customer Item to.
//
// Returns
// -------
//
function ciniki_poma_customerItemAdd(&$ciniki) {
    ...
    $rc = ciniki_poma_checkAccess($ciniki, $args['business_id'], 'ciniki.poma.customerItemAdd');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['business_id'], 'ciniki.poma.customeritem', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['business_id'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.poma.customerItem', 'object_id'=>$item_id));
